...
//	private static final BigInteger x = new BigInteger("37");
	...
	private static Ciphertext encrypt(BigInteger m) {
		...
		BigInteger hy = h.modPow(y, p);
		BigInteger c2 = hy.multiply(m).mod(p);
	...
	private static BigInteger decrypt(Ciphertext c) {
		BigInteger c2 = c.c2;
		BigInteger c1sk = c.c1.modPow(x, p);
		BigInteger c1_inversed = c1sk.modInverse(p);
		return c2.multiply(c1_inversed).mod(p);
	}
